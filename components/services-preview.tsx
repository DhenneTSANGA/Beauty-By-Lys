    ...
    <section className="mx-auto max-w-7xl px-5 py-20 md:px-8 md:py-28">
      ...
      <div className="mt-14 grid gap-6 md:grid-cols-3 md:gap-8">
        {services.map((s, i) => (
          <RevealOnScroll 
            key={s.title} 
            animation={i === 0 ? "animate-reveal-left" : i === 1 ? "animate-reveal-up" : "animate-reveal-right"}
            delay={`delay-${(i + 1) * 200}`}
            once={false}
          >
            <article
              className={`group relative flex flex-col overflow-hidden rounded-2xl border border-border bg-card transition-all hover:-translate-y-1 hover:shadow-xl hover:shadow-foreground/5 ${
